...
fn settings_flow() {
    ...
    run(async {
        ...
        let state: Value = invoke_json("workspace.get_local_state", workspace_args(id)).await;
        assert!(state["lastOpenedAt"].is_number());
        let state: Value = invoke_json(
            "workspace.set_last_open_file",
            json!({"workspaceId": id, "path": "notes/today.md"}),
        )
        .await;
        assert_eq!(state["lastOpenFile"], "notes/today.md");
