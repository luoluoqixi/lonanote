...
class LonanoteRustModulePackage : BaseReactPackage() {
  companion object {
    val JNI_PREPARE_MODULE_NAME = setOf(
      "__crabyLonanoteRustModule_JNI_prepare__"
    )
  }

  init {
    SoLoader.loadLibrary("cxx-lonanote-rust-module")
  }

  override fun getModule(name: String, reactContext: ReactApplicationContext): NativeModule? {
    if (name in JNI_PREPARE_MODULE_NAME) {
      nativeSetDataPath(reactContext.filesDir.absolutePath)
      return LonanoteRustModulePackage.TurboModulePlaceholder(reactContext, name)
    }
    return null
  }
