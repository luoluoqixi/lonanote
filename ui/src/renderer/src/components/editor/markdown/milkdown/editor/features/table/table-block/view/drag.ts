...
export function createDragRowHandler(refs: Refs, ctx?: Ctx) {
  return (event: DragEvent) => {
    handleDrag(
      ...
      event,
      ctx,
      ({ preview, content, previewRoot, yHandle, xHandle, colHandle, rowHandle }) => {
    ...
    if (info.type === 'col') {
      ...
      const col = children.find((col, index) => {
        ...
        if (direction === 'right') {
          ...
          if (boundaryLeft <= previewRight && boundaryRight >= previewRight) return true;
          if (index === firstRow.children.length - 1 && previewRight > boundaryRight) return true;
        } else {
          boundaryLeft = boundaryLeft - boundary.width / 2;
          boundaryRight = boundaryRight - boundary.width / 2;
          if (boundaryLeft <= previewLeft && boundaryRight >= previewLeft) return true;
...
export function useDragHandlers(refs: Refs, ctx?: Ctx, getPos?: () => number | undefined) {
