...
export const workspace = {
  importInitWorkspace: async (path: string): Promise<boolean> => {
    return path ? (await invoke("workspace.import_init_workspace", { path }))! : false;
  },
  isOpenWorkspace: async (path: string): Promise<boolean> => {
    return path ? (await invoke("workspace.is_open_workspace", { path }))! : false;
  },
  getWorkspace: async (path: string): Promise<Workspace | null> => {
    return path ? (await invoke("workspace.get_open_workspace", { path }))! : null;
  },
  getWorkspaceSettings: async (path: string): Promise<WorkspaceSettings> => {
    return (await invoke("workspace.get_open_workspace_settings", { path }))!;
  },
  setWorkspaceSettings: async (path: string, settings: WorkspaceSettings): Promise<Workspace> => {
    return (await invoke("workspace.set_open_workspace_settings", { path, settings }))!;
  },
  getOpenWorkspaceFileTree: async (path: string): Promise<FileTree> => {
    return (await invoke("workspace.get_open_workspace_file_tree", { path }))!;
  },
  setOpenWorkspaceFileTreeSortType: async (
    path: string,
    sortType: FileTreeSortType,
  ): Promise<void> => {
    return (await invoke("workspace.set_open_workspace_file_tree_sort_type", {
      path,
      sortType,
    }))!;
  },
  setOpenWorkspaceFollowGitignore: async (
    ...
    return (await invoke("workspace.set_open_workspace_follow_gitignore", {
      path,
      followGitignore,
    }))!;
  },
  setOpenWorkspaceCustomIgnore: async (path: string, customIgnore: string): Promise<void> => {
    return (await invoke("workspace.set_open_workspace_custom_ignore", {
      path,
      customIgnore,
    }))!;
  },
  resetOpenWorkspaceCustomIgnore: async (path: string): Promise<void> => {
    return (await invoke("workspace.reset_open_workspace_custom_ignore", {
      path,
    }))!;
  },
  resetOpenWorkspaceHistroySnapshootCount: async (path: string): Promise<void> => {
    return (await invoke("workspace.reset_open_workspace_histroy_snapshoot_count", {
      path,
    }))!;
  },
  resetOpenWorkspaceUploadAttachmentPath: async (path: string): Promise<void> => {
    return (await invoke("workspace.reset_open_workspace_upload_attachment_path", {
      path,
    }))!;
  },
  resetOpenWorkspaceUploadImagePath: async (path: string): Promise<void> => {
    return (await invoke("workspace.reset_open_workspace_upload_image_path", {
      path,
    }))!;
  },
  callOpenWorkspaceReinit: async (path: string): Promise<void> => {
    return (await invoke("workspace.call_open_workspace_reinit", { path }))!;
  },
  getOpenWorkspaceFileNode: async (
    ...
    return (await invoke("workspace.get_open_workspace_file_node", {
      path,
      nodePath,
      sortType,
      recursive,
