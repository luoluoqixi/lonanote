...
import { Button, NativeList, NativeListSection, NativeListItem, Text } from "@/components/ui";
...
async function loadPathDebugItems(): Promise<PathDebugItem[]> {
  const [dataDir, cacheDir, downloadDir, homeDir] = await Promise.all([
    path.getDataDir(), path.getCacheDir(), path.getDownloadDir(), path.getHomeDir(),
  ]);
  return [
        ...
        <NativeListItem>
          <View style={styles.header}>
            <View style={styles.headerText}>
              <Text fontSize="$5" fontWeight="600">路径测试</Text>
              <Text color="$color10" fontSize="$3">用于观察各平台默认路径。</Text>
            </View>
        ...
        {items.map((item) => (
          <NativeListItem key={item.key}>
            <View style={styles.item}>
              <Text color="$color10" fontSize="$2">{item.label}</Text>
...
const styles = StyleSheet.create({
  ...
  item: {
    borderColor: "rgba(128, 128, 128, 0.22)", borderRadius: 12,
    borderWidth: 1, gap: 4, paddingHorizontal: 12, paddingVertical: 8,
  },
});
