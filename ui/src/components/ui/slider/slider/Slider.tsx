...
import type { GestureReponderEvent, GetProps, SizeTokens, TamaguiElement } from "@tamagui/core";
import {
  getTokens,
  getVariableValue,
  styled,
...
const activeSliderMeasureListeners = new Set<() => void>();
...
function useOnDebouncedWindowResize(callback: () => void, amt = 200) {
  React.useEffect(() => {
    let last: any;
    const onResize = () => {
...
const getThumbSize = (val?: SizeTokens | number) => {
  ...
  const size =
    typeof val === "number"
      ? val
      : getSize((tokens.size as Record<string, any>)[String(val)] as any, {
          shift: -1,
        });
  return {
    width: size,
