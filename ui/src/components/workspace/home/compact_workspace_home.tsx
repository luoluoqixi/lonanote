import { type Href, useRouter } from "expo-router";
import { StyleSheet, View } from "react-native";
import { SafeAreaView } from "react-native-safe-area-context";
import { Button, ScrollView, Text, useTheme } from "rn-ui-kit";

import { os } from "@/api/common";
import { getAppHomeTitle } from "@/config";
import { useAppBackgroundColors } from "@/hooks/settings";

const DEBUG_HREF = "/debug" as Href;
const RECENT_HREF = "/recent" as Href;
const SETTINGS_HREF = "/settings" as Href;
const WORKSPACES_HREF = "/workspaces" as Href;

export function CompactWorkspaceHome() {
  const router = useRouter();
  const appBackgroundColors = useAppBackgroundColors();
  const theme = useTheme();
  const usesNativeHomeHeader = os() === "ios";

  return (
    <SafeAreaView
      edges={usesNativeHomeHeader ? ["left", "right", "bottom"] : ["top"]}
      style={styles.screen}
    >
      <ScrollView
        alwaysBounceVertical={usesNativeHomeHeader}
        contentContainerStyle={styles.content}
        contentInsetAdjustmentBehavior={usesNativeHomeHeader ? "automatic" : "never"}
        iosEmptyViewportScrollEnabled={usesNativeHomeHeader ? true : undefined}
        style={styles.scrollView}
        tracksNavigationBarScrollEdge
      >
        <View style={styles.intro}>
          {!usesNativeHomeHeader ? (
            <Text fontSize="$10" fontWeight="600">
              {getAppHomeTitle()}
            </Text>
          ) : null}
          <Text color="$color" opacity={0.66} fontSize="$3" lineHeight="$4">
            选择工作区、继续最近文档，或进入全局设置。
          </Text>
        </View>

        <View
          style={[
            styles.card,
            {
              backgroundColor: appBackgroundColors.card,
              borderColor: theme.borderColor.val,
            },
          ]}
        >
          <Text fontSize="$5" fontWeight="500">
            开始使用
          </Text>
          <Text color="$color" opacity={0.6} fontSize="$3" lineHeight="$4">
            从这里管理工作区、最近文档和应用设置。
          </Text>
          <Button onPress={() => router.push(WORKSPACES_HREF)}>选择工作区</Button>
          <Button onPress={() => router.push(RECENT_HREF)}>最近文档</Button>
          <Button onPress={() => router.push(SETTINGS_HREF)}>打开全局设置</Button>
          <Button onPress={() => router.push(DEBUG_HREF)}>打开 UI 调试界面</Button>
        </View>
      </ScrollView>
    </SafeAreaView>
  );
}

const styles = StyleSheet.create({
  screen: {
    flex: 1,
  },
  scrollView: {
    flex: 1,
  },
  content: {
    gap: 24,
    paddingHorizontal: 20,
    paddingVertical: 24,
  },
  intro: {
    gap: 12,
  },
  card: {
    gap: 12,
    borderRadius: 24,
    borderWidth: 1,
    padding: 16,
  },
});
