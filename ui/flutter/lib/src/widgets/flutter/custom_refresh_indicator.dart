...
// 修改了默认的 _kDragContainerExtentPercentage = 0.25, 默认的下拉触发距离太远了

// The over-scroll distance that moves the indicator to its maximum
// displacement, as a percentage of the scrollable's container extent.
const double _kDragContainerExtentPercentage = 0.05;
...
class CustomRefreshIndicator extends StatefulWidget {
  ...
  /// Creates a [CustomRefreshIndicator] with no spinner and calls `onRefresh` when
  /// successfully armed by a drag event.
  ///
  /// Events can be optionally listened by using the `onStatusChange` callback.
  const CustomRefreshIndicator.noSpinner({
    super.key,
    required this.child,
    required this.onRefresh,
    this.onStatusChange,
