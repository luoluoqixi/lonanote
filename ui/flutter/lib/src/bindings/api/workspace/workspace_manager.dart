...
class RustWorkspaceManager {
  ...
  static Future<void> setWorkspaceName(
      String path, String newName, bool isMove) async {
    await Bindings.invokeAsync(
  ...
  static Future<List<RustWorkspaceMetadata>> getWorkspacesMetadata() async {
    final res =
        await Bindings.invokeAsync(key: "workspace.get_workspaces_metadata");
  ...
  static Future<void> openWorkspaceByPath(String path) async {
    ...
    // }
    await Bindings.invokeAsync(
        key: "workspace.open_workspace_by_path", value: {"path": path});
  ...
  static Future<void> unloadWorkspaceByPath(String path) async {
    ...
    await Bindings.invokeAsync(
        key: "workspace.unload_workspace_by_path", value: {"path": path});
  }
  ...
  static Future<void> setWorkspaceSavedata(
      String workspacePath, RustWorkspaceSaveData data) async {
    return await Bindings.invokeAsync(
  ...
  static Future<void> test() async {
    ...
    logger
        .i("RustWorkspaceManager.getWorkspacesMetadata = ${workspaces.length}");

    final lastWorkspace = await getLastWorkspace();
    logger.i("RustWorkspaceManager.getLastWorkspace = $lastWorkspace");
  }
}
