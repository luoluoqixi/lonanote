...
class RustWorkspaceSaveData {
  RustWorkspaceSaveData({required this.lastOpenFilePath});
...
class RustWorkspaceData {
  RustWorkspaceData({required this.metadata, required this.settings});
...
@JsonSerializable()
class RustFileTree {
  RustFileTree({required this.path, required this.sortType, this.root});
  ...
  RustFileSortType getSortType() {
    try {
      ...
      if (kDebugMode) {
        print("parse sort type error: $sortType");
      }
...
enum RustFileTypeEnum { file, directory }
