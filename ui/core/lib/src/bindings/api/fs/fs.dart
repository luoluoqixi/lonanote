...
class RustFs {
  ...
  static List<String>? getFileList(String fullPath, {bool recursive = false}) {
    if (!exists(fullPath)) {
      return null;
    }
    if (!isDir(fullPath)) {
  ...
  static void test() async {
    if (kDebugMode) {
      ...
      print("RustFs.exists = ${exists(testFilePath)}");
      print("RustFs.readToString = $fileText");
      print("RustFs.readBinary = ${binary.length}");
      print("RustFs.isFile = ${isFile(testFilePath)}");

      final testDirPath = "${RustPath.getCacheDir()}/testDir";
      createDirAll(testDirPath);
      print("RustFs.createDirAll = ${exists(testDirPath)}");
      print("RustFs.isDir = ${isDir(testDirPath)}");
      delete(testDirPath, false);

      final copyFilePath = "${RustPath.getCacheDir()}/copyText.txt";
      copy(testFilePath, copyFilePath, true);
      print("RustFs.copy = ${exists(copyFilePath)}");

      final moveFilePath = "${RustPath.getCacheDir()}/moveText.txt";
      copy(testFilePath, moveFilePath, true);
      print("RustFs.move = ${exists(moveFilePath)}");
