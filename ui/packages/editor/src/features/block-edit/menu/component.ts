...
export const menuComponent: Component<MenuProps> = ({ show, hide, ctx, filter, config }) => {
  const { groups, size } = useMemo(() => getGroups(filter, config, ctx), [filter]);
          ...
          ${groups.map(
            (group) => html`
              <li
                ...
                class=${hoverIndex >= group.range[0] && hoverIndex < group.range[1]
                  ? 'selected'
                  : ''}
              >
                ${group.label}
