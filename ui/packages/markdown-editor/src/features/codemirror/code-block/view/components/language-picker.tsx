...
import { Fragment, computed, defineComponent, h, onMounted, onUnmounted, ref, watch } from 'vue';
  ...
  setup({ language, config, setLanguage, getAllLanguages, getReadOnly }) {
    ...
    const languages = computed(() => {
      ...
      const selected = all.find(
        // 暂未修复的bug, 未输入语言会导致language.value报空
        // ==== 修改 ====
            ...
            {showPicker.value ? (
              <div class="list-wrapper">
                <div class="search-box">
                  ...
                  />
                  <div
                    class={clsx('clear-icon', filter.value.length === 0 && 'hidden')}
                    onMousedown={(e) => {
                      e.preventDefault();
                      filter.value = '';
                    }}
                ...
                >
                  {!languages.value.length ? (
                    <li class="language-list-item no-result">{config.noResultText}</li>
                  ) : (
