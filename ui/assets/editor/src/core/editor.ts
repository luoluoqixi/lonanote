...
export class LonaEditor {
  ...
  destroy = () => {
    for (const k of Object.keys(this.#events) as Array<keyof LonaEditorEvent>) {
  ...
  canUndo = () => {
    return this.#editor ? undoDepth(this.#editor.state) > 0 : false;
  };

  canRedo = () => {
    return this.#editor ? redoDepth(this.#editor.state) > 0 : false;
  };
