...
- 建议下一阶段优先考虑以下优化：
	1. 为 `rust/core` 增加覆盖 workspace 主链路的后端集成测试。当前 UI 还没完全实现，很多 workspace 行为无法从页面层验证；可以优先用临时目录 + 真实文件系统夹具覆盖 `create/import/open/close/rename/move/remove/settings/savedata/roots sync/path relocation` 这几条核心链路，先把重构后的正确性锁住。
	2. `rust/core/src/workspace/file_tree/` 当前的 `unsafe` 构树实现可以视为“拥有型递归树 + 单次 Walk 流式构建”下的工程折中：它的作用域被限制在 `FileNode::from_path()` 内，实际只用于一次性构建新的树，不是需要立刻推翻的设计。后续真正需要重构它的时机，主要是 file tree 需要支持按层展开、懒加载、watcher 驱动的增量更新等更复杂能力时；到那时再重点评估是否改成显式 owned builder，例如递归构建、基于索引的 arena，或先收集节点再二次组装父子关系。
	3. 把 file tree 查询模型拆成“按层展开”和“全量递归展开”两类能力。当前 `recursive: bool` 只提供二元选择，后续如果要支持逐层展开文件夹、按需懒加载和“一次性展开全部”，更适合升级成 depth / expand strategy 一类更明确的 API，而不是继续把所有场景挤在一个布尔参数里。
	4. 预留 file tree 的增量更新模型，为后续文件监听自动刷新做准备。建议尽早明确：未来是继续整棵树重建，还是维护一个可增量 patch 的树；如果要接 watcher，最好补一层稳定节点标识、path 到 node 的索引，或至少定义一套 subtree refresh / patch 应用策略，避免监听接入后再被迫推翻当前结构。
